public class xComPutBlockPreview extends xCom {
    public String doCommand(String fullCommand) {
        ...
        if (factory.blockLoadMap.containsKey(blockString)) {
            ...
            args[3] = height;
            if(blockid.charAt(0) == '$') {
                int newid = sVars.getInt(blockid);
                blockid = Integer.toString(newid);
            }
            if(prefabid.charAt(0) == '$') {
                int newid = sVars.getInt(prefabid);
                prefabid = Integer.toString(newid);
            }
            if (rawX.charAt(0) == '$') {
                int transformedX;
        ...
        return String.format("put blockpreview %s id%s pid%s", blockString, blockid, prefabid);
    }

    private String escape() {
        return "usage: putblockpreview <BLOCK_TITLE> <id> <pid> <x> <y> <w> <h>. opt: <t> <m> ";
    }
}
