...
public class eGameLogicSimulation extends eGameLogicAdapter {
    ...
    private void updateThingPosition(gThing obj, long gameTimeMillis) {
            ...
            // TRACKS PLAYER
//            game.gPlayer player = game.xMain.shellLogic.getUserPlayer();
//            if(player != null) {
...
//            }
    ...
    private void updateEntityPositions(long gameTimeMillis) {
        ...
        for(String id : svMap.keys()) {
            ...
            // add a "collidedPlayer" arg to game.gThing and get velocity
...
//            game.xMain.shellLogic.console.ex(String.format("fireweapon %s %d", src.id, src.weapon));
        }
        return closest;
    }
    ...
    private void checkBulletSplashes() {
        ...
        for(gPlayer p : bulletsToRemovePlayerMap.keySet()) {
            ...
            //calculate dmg
//            int dmg = b.dmg - (int)((double)b.dmg/2 *((Math.abs(Math.max(0, game.sSettings.gameTime - b.timestamp))/(double)b.ttl))); // dmg falloff based on age of bullet
